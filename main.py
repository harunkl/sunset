...
def get_saved_name(number: int) -> str:
    # pastikan tokens terbaru sudah diload
    AuthInstance.load_tokens()
    rt = next((x for x in AuthInstance.refresh_tokens if x.get("number") == int(number)), None)
    return rt.get("name", "Tanpa Nama") if rt else "Tanpa Nama"
...
def show_main_menu(profile):
    ...
    print(f"Nomor: {profile['number']} ({profile['name']}) | Type: {profile['subscription_type']}".center(WIDTH))
    ...
    while True:
        ...
        if active_user is not None:
            ...
            profile = {
                ...
                "name": get_saved_name(active_user["number"]),
