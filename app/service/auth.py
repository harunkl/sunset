...
class Auth:
    ...
    def add_refresh_token(self, number: int, refresh_token: str, name: str | None = None):
        # Normalisasi name
        name_clean = (str(name).strip() if name is not None else "")
        if name_clean == "":
            name_clean = "Tanpa Nama"

        # Cari existing user
        existing = next((rt for rt in self.refresh_tokens if rt.get("number") == int(number)), None)

        if existing:
            existing["refresh_token"] = refresh_token
            # hanya update name kalau name dikasih
        ...
        # Set aktif
        self.set_active_user(int(number))
